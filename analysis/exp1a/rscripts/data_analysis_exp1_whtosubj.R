...
d <- subset(d, test_condition == "SUBJ")
...
sink(file="generalization_model_mismatch_subject.txt")
summary(phase_model)
sink(file=NULL)
...
# save output to a txt file
sink(file="generalization_model_match_subject.txt")
summary(phase_model2)
sink(file=NULL)
...
d_no_fillers <- subset(d_no_ungram, item_type != "FILL")
d_no_fillers$item_type <- factor(d_no_fillers$item_type, c("SUBJ", "WH"))
d_no_fillers$exposure_condition <- factor(d_no_fillers$exposure_condition, c("WH", "SUBJ"))
...
phase_graph<- ggplot(phase_avg, aes(x=exposure_condition,y=Mean, fill=exposure_condition)) +
  ...
  xlab("exposure type") +
  ylab("average acceptability of subject islands") +
  theme_bw() 

phase_graph
ggsave("../graphs/exp1_phase_bars_subject.pdf",plot=phase_graph,width=10,height=5)
ggsave("../graphs/exp1_phase_bars_subject.png",plot=phase_graph,width=10,height=5)
...
ggsave("../graphs/exp1_phase_curves_subject.png",width=10,height=5)
...
curve_grouped
ggsave("../graphs/exp1_curves_subject.pdf",width=10,height=5)
ggsave("../graphs/exp1_curves_subject.png",width=10,height=5)




# overall chart with fillers
############################ 
...
ggsave("../graphs/satiation_exp1_plot_subject.pdf",width=10,height=5)
ggsave("../graphs/satiation_exp1_plot_subject.png",width=10,height=5)
...
ggsave("../graphs/subject_variability_exp1_subject.pdf", width=20, height = 25)
ggsave("../graphs/subject_variability_exp1_subject.png", width=20, height = 25)
