...
num_exposure_items = 20;
num_test_items = 6;
num_pre_exposure_items = 6;
num_unique_lex_items = num_exposure_items + num_test_items + num_pre_exposure_items;
num_fillers = 16; // half the number of target items
block_size = 4;
num_exposure_blocks = num_exposure_items/2;         // 2 items per block
num_test_blocks = num_test_items/2;                 // 2 items per block
num_pre_exposure_blocks = num_pre_exposure_items/2; // 2 items per block
...
// pre-exposure/test items are the same type, so we can pull them together
test_preexp_pool = all_stimuli.filter(function (e){
...
// length should be num_exposure_items
exposure_stimuli = shuffle(exposure_pool).slice(0,num_exposure_items);
// length should be num_test_items + num_pre_exposure_items
test_preexp_stimuli = test_preexp_pool.filter(function (t){
...
fillers_ungram = all_stimuli.filter(function (e){
  return e.condition == "UNGRAM" && e.lex_items < num_fillers+(101+num_fillers);
});
...
// these draw from the same pool, but there should be enough
// items in the pool to fill all the blocks exactly
test_blocks = []; // a list of lists of item objects
for(var i = 0; i < num_test_blocks; i++){
  block = [test_preexp_stimuli.pop(), test_preexp_stimuli.pop(), fillers_gram.pop(), fillers_ungram.pop()];
  block = shuffle(block);
  test_blocks.push(block);
}
pre_exposure_blocks = []; // a list of lists of item objects
for(var i = 0; i < num_test_blocks; i++){
  block = [test_preexp_stimuli.pop(), test_preexp_stimuli.pop(), fillers_gram.pop(), fillers_ungram.pop()];
  block = shuffle(block);
  pre_exposure_blocks.push(block);
}
...
for(var i = 0; i < num_pre_exposure_blocks; i++){
  for (var j = 0; j < block_size; j++){

    pre_exposure_blocks[i][j]["new_block_sequence"] = i+201;
    pre_exposure_blocks[i][j]["exposure_condition"] = exposure_cond;
    pre_exposure_blocks[i][j]["test_condition"] = test_cond;
    pre_exposure_blocks[i][j]["phase"] = "pre-exposure";
    pre_exposure_blocks[i][j]["group"] = experiment_group;

  };
};

// create final sequence to present to the slide
final_item_sequence = [pre_exposure_blocks, exposure_blocks, test_blocks].flat().flat();
